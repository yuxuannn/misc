...
def scrapenode(elem, tag):
    ...
    fullvuln=fullvuln.replace('\r', ' ')
    fullvuln=fullvuln.replace('\n', ' ')
    return fullvuln
...
def extractnexposevulndefns2(root):
    ...
    fields=['id', 'Risk', 'cvssScore', 'cvssVector', 'Title', 'Description', 'Solution', 'Exploits','References',  'Tags', 'Malware', 'Malware-Raw', 'Description-Raw']
...
def extractnexposefindings2(root):
    ...
    fields=['Scan ID', 'Test ID', 'Test Output', 'Full Test Output', 'IP Address', 'Port', 'Service Name', 'Protocol', 'HostPort','FullHostPort','Hostnames', 'Vuln Attributes', 'Node Attributes']
    ...
    for vuln in root.findall("./nodes/node/tests/test"):
        ...
        listofresult.append([vuln.attrib.get('scan-id'), vuln.attrib.get('id'), fullvuln, ET.tostring(vuln), ip, port, servicename, proto, hostport, protohostport, hostnames, vuln.attrib, vuln_node.attrib])
    ...
    for vuln in root.findall("./nodes/node/endpoints/endpoint/services/service/tests/test"):
        ...
        proto=endpoint.attrib.get('protocol').upper() # for insightvm, convert 'tcp' to 'TCP'
        #hostport=ip+':'+port+' ('+proto+')'
        hostport=ip+':'+port
        protohostport=ip+' ('+proto+'/'+port+')'
...
def nexpose2csv() :
    ...
    # 'Scan ID', 'Test ID', 'Test Output', 'Full Test Output', 'IP Address',
    #   'Port', 'Service Name', 'Protocol', 'HostPort', 'Hostnames',
    #   'Vuln Attributes', 'Node Attributes'
    ...
    df4.to_excel(writer, sheet_name='Raw')
    #writer.save()
    ...
    df4['Service Name']=df4['Service Name'].map(mapsvc)

    # Aggregate Affected IP Addr & Port by Assset Type, Service Name, Test ID and Evidence
    # sort by ip first before concatenate to maintain consistency
    df4=df4.sort_values(by=['AssetType','Service Name', 'Test ID', 'Test Output', 'IP Address'])
    ...
    df4[['HostPort']]=pd.DataFrame(df4.groupby(['AssetType','Service Name', 'Test ID', 'Test Output'])['HostPort'].transform(lambda x: '\n'.join(x.unique())))
    df4[['IP Address']]=pd.DataFrame(df4.groupby(['AssetType','Service Name', 'Test ID', 'Test Output'])['IP Address'].transform(lambda x: '\n'.join(x.unique())))
    #df4[['HostPort']]
    #df4=df4[['Service Name', 'Test ID', 'Test Output','HostPort','AssetType']].drop_duplicates().reset_index(drop=True)
    #df4=df4[['Service Name', 'Test ID', 'Test Output','IP Address','HostPort','AssetType']].drop_duplicates().reset_index(drop=True)
    ...
    df5=df5.sort_values(by=['AssetType','Service Name', 'Test ID', 'IP Address'])
    df5[['IP Address','HostPort','Test Output']]=pd.DataFrame(df5.groupby(['AssetType','Service Name', 'Test ID'])['IP Address','HostPort','Test Output'].transform(lambda x: '\n'.join(x.unique())))
    df5=df5[['Service Name', 'Test ID', 'Test Output','IP Address','HostPort','AssetType']].drop_duplicates().reset_index(drop=True)
    ...
    df6['Name'] = df6['title']
    df6=df6[['Risk','CVSS3 Base Score','Service Name','Test ID','title','Description','Solution','AssetType','IP Address','HostPort','Name','Test Output','CVSS3 Vector','cvssScore','cvssVector','Exploits','References','Tags','Malware','Malware-Raw','Description-Raw']]
    ...
    df6=df6.sort_values(by=['Service Name','Test ID'])


    df6.to_csv(outputFile1)
    #df6.to_excel(outputFile2)
    df6.to_excel(writer, sheet_name='Compressed')
    writer.save()
...
def nexpose2csv2() :
    ...
    outputFile1=os.path.join(baseFolder, filename+'-results.csv')
    outputFile2=os.path.join(baseFolder, filename+'-resultsforanalysis.xlsx')
    outputFile3=os.path.join(baseFolder, filename+'-resultsforreporting.csv')
    tree=ET.parse(inputFile)
    root=tree.getroot()    

    
    # 'Scan ID', 'Test ID', 'Test Output', 'Full Test Output', 'IP Address',
    #   'Port', 'Service Name', 'Protocol', 'HostPort', 'FullHostPort', 'Hostnames',
    #   'Vuln Attributes', 'Node Attributes'
    df1=extractnexposefindings2(root)
    df1=df1.iloc[np.argsort(list(map(socket.inet_aton,df1['IP Address'])))].reset_index(drop=True)
    #df1['FullHostPort']=df1['IP Address']+':'+df1['Port']+' ('+df1['Protocol']+')'
    #df1['HostPort']=df1['IP Address']+':'+df1['Port']
    #hostport=ip+':'+port+' ('+proto+')'
    #hostport=ip+' ('+proto+'/'+port+')'
    # fields=['id', 'Risk', 'title', 'Description', 'Solution','cvssScore', 'cvssVector', 
    # 'Exploits','References',  'Tags', 'Malware', 'Malware-Raw', 'Description-Raw']
    df2=extractnexposevulndefns2(root)
    df3=pd.merge(df1,df2, left_on='Test ID', right_on='id')
    df3=df3.sort_values(by=['Test ID']).reset_index(drop=True)
    df3.to_csv(outputFile1)


    #with pd.ExcelWriter(outputFile2) as writer:
    df1=df1.sort_values(by=['Service Name','Test ID']).reset_index(drop=True)
    writer=pd.ExcelWriter(outputFile2)
    df1.to_excel(writer, sheet_name='Raw')

    df4=df1[['Service Name','Test ID', 'Test Output', 'IP Address','Port','Protocol', 'HostPort', 'FullHostPort', 'Hostnames']]
    # Sort by <Service Name, Test ID>
    df4=df4.sort_values(by=['Service Name','Test ID']).reset_index(drop=True)
    #df4=df4.assign(AssetType=df4['Service Name'])
    # Add New Column, Asset Type
    ...
    # sort by ip first before concatenate to maintain consistency
    #df4=df4.sort_values(by=['AssetType','Service Name', 'Test ID', 'Test Output', 'IP Address'])
    df4=df4.sort_values(by=['AssetType','Service Name', 'Test ID', 'Test Output'])
    df4[['HostPort']]=pd.DataFrame(df4.groupby(['AssetType','Service Name', 'Test ID', 'Test Output'])['HostPort'].transform(lambda x: '\n'.join(x.unique())))
    df4[['IP Address']]=pd.DataFrame(df4.groupby(['AssetType','Service Name', 'Test ID', 'Test Output'])['IP Address'].transform(lambda x: '\n'.join(x.unique())))
    ...
    #df7=df7.sort_values(by=['Test ID', 'Test Output', 'IP Address'])
    df7=df7.sort_values(by=['Test ID', 'Test Output'])
    df7[['IP Address','HostPort','FullHostPort']]=pd.DataFrame(df7.groupby(['Test ID','Test Output'])['IP Address','HostPort','FullHostPort'].transform(lambda x: ', '.join(x.unique())))
    #df7['Test Output']=df7['Test Output']+'\n'+df7['HostPort']
    df7['Test Output']="Test Output for [" + df7['FullHostPort'] +"]: " +'\n' + df7['Test Output'] +'\n'
    df7[['Test Output']]=pd.DataFrame(df7.groupby(['Test ID'])['Test Output'].transform(lambda x: '\n'.join(x.unique())))
    df7[['IP Address','HostPort','FullHostPort']]=pd.DataFrame(df7.groupby(['Test ID'])['IP Address','HostPort','FullHostPort'].transform(lambda x: ', '.join(x.unique())))
    df7=df7[['Test ID', 'Test Output','IP Address','HostPort','FullHostPort']].drop_duplicates().reset_index(drop=True)
    df8=pd.merge(df7,df2, left_on='Test ID', right_on='id')
    df8.to_csv(outputFile3)


    print('Output CSV File: '+outputFile1)
    print('Output Excel File: '+outputFile2)
    print('Output CSV File: '+outputFile3)
...
def nessus2csv():
    ...
    #df1=df[['Plugin', 'Plugin Name', 'Family', 'Severity', 'CVSS V3 Base Score','CVSS V3 Vector', 'Risk Factor','IP Address','Port','Protocol','Plugin Text','Synopsis', 'Description', 'Solution', 'See Also','BID', 'Cross References', 'Exploit Ease', 'Exploit Frameworks','Exploit?','CVSS V2 Base Score', 'CVSS V2 Temporal Score','CVSS V2 Vector', 'CVE', 'First Discovered', 'Last Observed','Vuln Publication Date', 'Patch Publication Date','Plugin Publication Date', 'Plugin Modification Date', 'Repository', 'MAC Address', 'DNS Name','NetBIOS Name']]
    df2=df[['Plugin', 'Plugin Name', 'Family', 'Severity', 'CVSS V3 Base Score','CVSS V3 Vector', 'Risk Factor','IP Address','Port','Protocol','Plugin Text','Synopsis', 'Description', 'Solution', 'See Also']]

    #df2.columns=['Plugin', 'Test ID', 'Family', 'Severity', 'CVSS V3 Base Score','CVSS V3 Vector', 'Risk Factor','IP Address','Port','Protocol','Test Output','Synopsis', 'Description', 'Solution', 'See Also']
    
    df2['CVSS V3 Base Score']=df2['CVSS V3 Base Score'].fillna(0)
    df2.fillna("", inplace=True)
    #df2['Plugin Text']=df2['Plugin Text'].fillna("")
    #df2['Solution']=df2['Solution'].fillna("")
    #df2['CVSS V3 Base Score']=df2['CVSS V3 Base Score'].astype(str)
    df2['IP Address']=df2['IP Address'].astype(str)
    df2['HostPort']=df2['IP Address']+':'+df2['Port'].astype(str)
    df2['FullHostPort']=df2['IP Address']+' ('+df2['Protocol']+'/'+df2['Port'].astype(str)+')'
    df2['Test Output']=df2['FullHostPort']+'\n'+df2['Plugin Text']
    df2['Test Output']=df2['Test Output'].astype(str)
    df2['Title']=df2['Plugin Name']
    df2['Solution']=df2['Solution']+'\n'+'Ref: '+df2['See Also']
    df2=df2[['Plugin', 'Title', 'Family', 'Severity', 'CVSS V3 Base Score','CVSS V3 Vector', 'Risk Factor','IP Address','HostPort','FullHostPort','Test Output','Synopsis', 'Description', 'Solution', 'See Also']]
    df2
    df7=df2
    df7=df7.sort_values(by=['Title'])
    df7[['IP Address','HostPort','FullHostPort']]=pd.DataFrame(df7.groupby(['Title'])['IP Address','HostPort','FullHostPort'].transform(lambda x: ', '.join(x.unique())))
    df7[['Test Output']]=pd.DataFrame(df7.groupby(['Title'])['Test Output'].transform(lambda x: '\n\n'.join(x)))
    df7=df7.drop_duplicates().reset_index(drop=True)
    df7=df7.sort_values(by=['CVSS V3 Base Score', 'Family','Title'], ascending=[False, True, True])
    df7=df7.drop_duplicates().reset_index(drop=True)
    print(df7['CVSS V3 Base Score'])
    #df7['CVSS V3 Base Score']=df7['CVSS V3 Base Score'].astype(str)
    df7['Severity']=df7['CVSS V3 Base Score'].map(CVSSSeverity2)
    df7.shape
    df7.to_csv(outputFile3)
    df7.to_excel(outputFile2)
    print('Output CSV File: '+outputFile1)
    print('Output Excel File: '+outputFile2)
    print('Output CSV File: '+outputFile3)
...
if len (sys.argv) < 2 :
    ...
    print("Usage: "+os.path.basename(__file__) +" <nessus output csv>")
    print("Exiting ...")
    sys.exit (1)
else:
    nessus2csv()